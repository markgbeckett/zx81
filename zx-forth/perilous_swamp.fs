...
( KILLED, OR ZERO IF NOT )
( IDENTIFIERS ARE AS IN ZX81 BASIC ORIGINAL )
: FIGHT ( BEAST POINTS -- BEAST FLAG )
    ( ADJUST POINTS FOR WEIGHT CARRIED )
    GEMS @ 3 * 100 / - 10 * ( K )
    ( COMPUTE BEASTS COMBAT POINTS )
    OVER 10 * ( M )
    ( FIGHT PARAMS )
    1000 RND ( I' )
    20 ( L )
...
: ISMOUSE ( MONSTER TREASURE FLAG -- MONSTER TREASURE FLAG )
    3 PICK 0 = IF ( IS A MOUSE )
	" A SMALL GREY MOUSE " SCC .W
...
: HANDLELOC ( X Y -- X Y )
    ...
    BEGIN
	1 ISMOUSE ( CHECK FOR MOUSE )
    WHILE
	    ( CHECK IF WIZARD HAS PET )
	    ( FIGHT, BRIBE, RUN )
	    DISPACT
	    GETACT
	    CASE FIGHT RUN BRIBE ZAO *** BAT OUT
    REPEAT

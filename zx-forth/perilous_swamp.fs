...
: 1+ 1 + ;
: 1- 1 - ;
: PAUSE S->D TIME D@ + BEGIN 2DUP TIME D@ D< UNTIL DROP DROP ;
...
: RAND ?DUP 0 = IF 31836 @
	SWAP THEN SEED ! ;
: CHECKSTACK 0 SWAP DO I PICK . -1 +LOOP CR ;
-->
...
0 VARIABLE CHOICE ( USED TO CHECK IF PLAYER HAS CHOICE OR MUST FIGHT )
0 VARIABLE WIN ( USED TO CONFIRM MONSTER KILLED )
...
: MVPLAYER ( COL ROW DCOL DROW -- COL ROW FLAG )
...
: MVP ( X Y -- NX NY )
    BEGIN 
	GETDIR DUP -1 = WHILE DROP REPEAT
    UNTIL
...
: PRCMPSS ( -- )
    ...
    "  W+E" SCC .W SCC SCCR
    "   S" SCC .W SCC SCCR
    " X=YOU" SCC .W SCC SCCR
    PRINCESS HAS 0= IF " P=PRINCESS" SCC .W THEN
    SCC SCCR
    " #=SWAMP" SCC .W SCC SCCR
    " #=EDGE" SCC .W SCC SCCR
    SCC SCCR
    "STRENGTH=" SCC .W STRENGTH @ # SCC .W SCC SCCR
    "TREASURE=" SCC .W GEMS @ # SCC .W SCC SCCR
;
...
: DESCMON ( BEAST TREASURE -- BEAST TREASURE )
    ...
    ."  IS GUARDING " DUP TREAS W@ C0 .W CR

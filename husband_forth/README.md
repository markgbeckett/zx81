...
* First, ZX81-Forth does not enable Z80 interrupt mode 1, which is critical to the correct operation of the monitor. The display routine in ZX81-Forth relies on the maskable interrupt routine at 0x38 to display text and, to use this routine, the Z80 needs to be configured for `IM 1`. This puzzled me at first, as I did not understand how the monitor worked on the ZX81. However, an experiment with a real ZX81 showed that, even in interrupt mode 0, it accesses the routine at 0x38. I infer that the ZX81 puts the code for `rst 0x38` (op-code 0xFF) on the database. The Minstrel 3 does not (though could be modified to do so). To correct this, I inserted an `im 1` command into the initialisation code.

* Second, ZX81-Forth relies on read and write operations to upper memory (0x8000 onwards) functioning. The ZX81 hardware intercepts attempts to execute code in upper memory as part of its display-handling code. However, it does not intercept read and write operations. In contrast, the Minstrel 3 intercepts read, write, and execute operations, so reading/ writing to upper memory (which is typically a shadow copy of lower memory) is not possible. To address this, I refactored the code so that it used only the lower half of memory. Note that, because of how RAM shadowing works, this does not typically reduce the memory available to the routine.

* Third, ZX81-Forth tries to compute how much RAM a computer has at boot time, to allow it to accommodate a range of RAM configurations. The routine effectively counts down from the top of memory (0xFFFF) until it finds the end of the last shadow copy of RAM. In this way, it computes the amount of RAM installed above 0x4000. It then checks, separately, from RAM in 0x2000--0x3FFF. Because of the memory-access difference noted in the previous point, and because the routine is not completely resilient (i.e., it also does not work with the default ZXpand configuration), it reports the wrong amount of RAM. I have addressed this by hard-wiring the value for upper memory to be 16kB (which, plus the RAM in 0x2000--0x3FFF) means ZX81-Forth on the Minstrel 3 has 24kB of RAM to work with.

Whilst the display routine would work as well on the Minstrel 3 as on the ZX81, I do not think it is a very good match to the PAL standard. In particular, it has too few scanlines and the end of main-display scanlines does not properly contain a right-hand border. I addressed the frame-size issue by increasing the border slightly, in the hope of producing a more stable display. I have yet to tackle the right-hand border issue, as this requires more significant reworking. At present, the right-hand end of each line of text is likely to be inverted and may be corrupted.
...
- Byte 10 is the task status byte. The lower six bytes of this contain the backlog of task instances to be run. If the lower six bits are non-zero then the task is due to run (potentially multiple runs, if the scheduler is overloaded with tasks).

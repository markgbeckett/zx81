...
* First, ZX81-Forth does not enable Z80 interrupt mode 1, which is critical to the correct operation of the monitor. The display routine in ZX81-Forth relies on the maskable interrupt routine at 0x38 to display text and, to use this routine, the Z80 needs to be configured for `IM 1`. This puzzled me at first, as I did not understand how the monitor worked on the ZX81 (when using the default interrupt mode 0). However, an experiment with a real ZX81 showed that, even in (the default) interrupt mode 0, it accesses the routine at 0x38. I infer that the ZX81 puts the code for `rst 0x38` (op-code 0xFF) on the databus. The Minstrel 3 does not (though could be modified to do so). To correct this, I inserted an `im 1` command into the initialisation code.

* Second, ZX81-Forth relies on read and write operations to upper memory (0x8000 onwards) functioning. The ZX81 hardware intercepts attempts to execute code in upper memory as part of its display-handling code. However, it does not intercept read and write operations. In contrast, the Minstrel 3 intercepts read, write, and execute operations, so reading/ writing to upper memory (which is typically a shadow copy of lower memory) is not possible. To address this, I refactored the code so that it used only the lower half of memory. Note that, because of how RAM shadowing works, this does not typically reduce the memory available for use by ZX81-Forth.

* Third, ZX81-Forth tries to compute how much RAM a computer has at boot time, to allow it to accommodate a range of RAM configurations. The routine effectively counts down from the top of memory (0xFFFF) until it finds the end of the last shadow copy of RAM. In this way, it computes the amount of RAM installed above 0x4000. It then checks, separately, for RAM in 0x2000--0x3FFF. Because of the memory-access difference noted in the previous point, and because the routine is not completely resilient (i.e., it also does not work with the default ZXpand configuration), it reports the wrong amount of RAM. I have addressed this by hardwiring the value for upper memory to be 16kB (which, plus the RAM in 0x2000--0x3FFF) means ZX81-Forth on the Minstrel 3 has 24kB of RAM to work with.
